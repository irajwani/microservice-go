...
# -----------------------------------------------
# Go Lambda (Hello World) - kept alongside Python lambda
# Builds the Go binary from ../main.go and deploys as a separate Lambda
# -----------------------------------------------

resource "null_resource" "build_go_lambda" {
  # Rebuild when source changes
  triggers = {
    source_hash = filesha256("${path.module}/../main.go")
  }

  provisioner "local-exec" {
    # Build linux binary required by Lambda
...
resource "aws_lambda_function" "hello_go_lambda" {
  # Minimal hello-world Go Lambda
  function_name = var.go_lambda_name
  handler       = "hello"   # matches built binary name
  runtime       = "go1.x"
  role          = aws_iam_role.lambda_execution_role.arn
...
resource "aws_cloudwatch_log_group" "GoHelloLambdaLogGroup" {
  name              = "/aws/lambda/${aws_lambda_function.hello_go_lambda.function_name}"
  retention_in_days = 1
}
